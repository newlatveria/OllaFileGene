...
# Returns "used total avail%" for the filesystem that owns $1
get_disk_info() {
    local path="$1"
    if [[ -e "$path" ]]; then
        df -h "$path" | awk 'NR==2 {print $3, $2, $4, $5}'
    else
        echo "N/A N/A N/A N/A"
    fi
}

# Stop ollama; run "$1"; restart ollama.  Returns the exit code of "$1".
with_service_stopped() {
    local service_was_running=false
    if check_ollama_running 2>/dev/null; then
        service_was_running=true
        echo -e "${YELLOW}Stopping Ollama service...${STD}"
    ...
    if [[ "$service_was_running" == true ]]; then
        echo -e "${YELLOW}Restarting Ollama service...${STD}"
        if command -v systemctl &> /dev/null; then
...
# ---------------------------------------------------------------------------
# Ensure the models directory (and its parents down to ~/.ollama) exist and
# are owned by the user who is running this script.  The ollama systemd unit
# runs as root, so if root created any of these directories they will not be
# writable by the user — which is the most common cause of "permission denied"
# errors on pull / create.
# ---------------------------------------------------------------------------
fix_models_ownership() {
    local target="${1:-$OLLAMA_MODELS_DIR}"

    # Create the full path if it does not exist yet
    mkdir -p "$target"

    # Determine the real user even when the script is invoked via sudo
    ...
    # If anything under ~/.ollama is owned by root (uid 0) fix it
    if [[ -d "$HOME/.ollama" ]]; then
        local root_owned
        root_owned=$(find "$HOME/.ollama" -maxdepth 0 -uid 0 2>/dev/null)
        if [[ -n "$root_owned" ]]; then
            sudo chown -R "${real_uid}:${real_gid}" "$HOME/.ollama"
            log_success "Fixed ownership of $HOME/.ollama → $real_user"
        fi
    fi

    # Same for the models directory itself (may be outside ~/.ollama)
    local root_models
    root_models=$(find "$target" -maxdepth 0 -uid 0 2>/dev/null)
    if [[ -n "$root_models" ]]; then
        sudo chown -R "${real_uid}:${real_gid}" "$target"
        log_success "Fixed ownership of $target → $real_user"
    fi
}
...
install_ollama() {
    ...
    if curl -fsSL https://ollama.com/install.sh | sh; then
        ...
        # The installer / first service start may have created ~/.ollama as
        # root.  Fix ownership so the current user can pull models.
        fix_models_ownership "$OLLAMA_MODELS_DIR"
...
model_pull() {
    ...
    # Ensure the blobs directory exists and is owned by this user.
    # The ollama service (often root) is the process that actually writes
    # the blob files, so if the directory was created by root the pull will
    ...
    if ollama pull "$model_name"; then
        ...
        echo ""
        echo -e "${YELLOW}If you see a 'permission denied' error above, try:${STD}"
        echo "  sudo chown -R $(whoami) $OLLAMA_MODELS_DIR"
...
restore_models() {
    ...
    if tar -xzf "$BACKUP_DIR/$backup_file" -C "$(dirname "$OLLAMA_MODELS_DIR")"; then
        # tar may have extracted files as root; fix before the service restarts
        fix_models_ownership "$OLLAMA_MODELS_DIR"
...
# =============================================================================
# MODULE: MODEL STORAGE MANAGEMENT
# =============================================================================

# ── helpers ──────────────────────────────────────────────────────────────────

# Pretty-print the contents of $OLLAMA_MODELS_DIR with per-model size.
# Falls back to a raw listing when Ollama is not running / no blobs exist.
storage_list_models() {
    echo ""
    echo -e "${BOLD}  Models directory : ${CYAN}$OLLAMA_MODELS_DIR${STD}"

    read -r used total avail pct <<< "$(get_disk_info "$OLLAMA_MODELS_DIR")"
    echo -e "${BOLD}  Disk            : ${STD}used ${GREEN}${used}${STD}  total ${GREEN}${total}${STD}  free ${GREEN}${avail}${STD}  (${pct} used)"
    echo ""

    # If the service is up we can rely on `ollama list` for the name↔digest
    # mapping; otherwise fall back to walking the blob directory.
    if check_ollama_running 2>/dev/null; then
        ollama list 2>/dev/null
        echo ""
        # Show raw directory total so the user can see blob-level usage
        echo -e "  ${BOLD}Total on disk:${STD} $(du -sh "$OLLAMA_MODELS_DIR" 2>/dev/null | cut -f1)"
    else
        echo -e "  ${YELLOW}(service offline – showing raw directory tree)${STD}"
        if [[ -d "$OLLAMA_MODELS_DIR" ]]; then
            find "$OLLAMA_MODELS_DIR" -maxdepth 3 -type f -printf '  %s\t%p\n' 2>/dev/null | \
                sort -rn | head -30 | \
                awk '{printf "  %8s  %s\n", ($1>1073741824 ? sprintf("%.2f GB",$1/1073741824) : \
                      ($1>1048576 ? sprintf("%.2f MB",$1/1048576) : sprintf("%.2f KB",$1/1024))), $2}'
            echo ""
            echo -e "  ${BOLD}Total on disk:${STD} $(du -sh "$OLLAMA_MODELS_DIR" 2>/dev/null | cut -f1)"
        else
            echo -e "  ${RED}Directory does not exist${STD}"
        fi
    fi
    echo ""
}
...
storage_view() {
    clear
    echo -e "${CYAN}=== Storage — Current State ===${STD}"
    storage_list_models
    pause
}

# Move every blob & manifest under $OLLAMA_MODELS_DIR to a new path, then
# re-point OLLAMA_MODELS (and the systemd unit if present) at the new path.
storage_move() {
    clear
    echo -e "${CYAN}=== Storage — Move Models Directory ===${STD}"
    echo ""
    echo -e "  Current directory : ${CYAN}$OLLAMA_MODELS_DIR${STD}"
    if [[ -d "$OLLAMA_MODELS_DIR" ]]; then
        echo -e "  Current size      : $(du -sh "$OLLAMA_MODELS_DIR" 2>/dev/null | cut -f1)"
    fi
    echo ""
    echo "  Enter the NEW path.  It may already exist (contents will be"
    echo "  merged) or it will be created automatically."
    echo ""

    read -r -p "  New models path: " new_path
    ...
    if [[ "$new_path" == "$OLLAMA_MODELS_DIR" ]]; then
        log_error "New path is the same as the current path"
        pause
        return
    fi

    # Warn if destination already has files
    if [[ -d "$new_path" ]] && [[ -n "$(ls -A "$new_path" 2>/dev/null)" ]]; then
        echo -e "${YELLOW}WARNING: $new_path already contains files.${STD}"
        echo "  Existing blobs will NOT be overwritten; new blobs will be added."
        if ! confirm_action "  Continue anyway?"; then
            return
        fi
    fi

    # Show destination disk space
    # (use / as fallback when $new_path does not yet exist)
    local check_path="$new_path"
    [[ ! -e "$check_path" ]] && check_path="/"
    read -r used total avail pct <<< "$(get_disk_info "$check_path")"
    echo -e "  Destination disk : used ${used}  total ${total}  free ${avail}  (${pct} used)"
    echo ""

    if ! confirm_action "  Move models from $OLLAMA_MODELS_DIR  →  $new_path ?"; then
        return
    fi

    log_msg "Moving models: $OLLAMA_MODELS_DIR -> $new_path"

    # Create destination and make sure it is user-owned before rsync runs
    mkdir -p "$new_path"
    fix_models_ownership "$new_path"

    # --- do the heavy copy/move with service offline ---
    with_service_stopped "
        echo -e '  ${CYAN}Copying blobs…${STD}'
        # rsync preserves hard-links within the tree so duplicate blobs do not
        # eat extra space at the destination.
        if ! rsync -ahP --progress \"\$OLLAMA_MODELS_DIR/\" \"$new_path/\"; then
            echo 'RSYNC_FAILED'
            exit 1
        fi
        echo -e '  ${CYAN}Removing old directory…${STD}'
        rm -rf \"$OLLAMA_MODELS_DIR\"
    "
    local rc=$?

    if [[ $rc -ne 0 ]]; then
        log_error "Move failed (rsync exited $rc). Old directory may still exist."
        pause
        return
    fi

    # rsync may have run as the current user but some files could still be
    # root-owned from the old location; normalise ownership now.
    fix_models_ownership "$new_path"

    # --- update runtime + persisted config ----
    export OLLAMA_MODELS="$new_path"
    OLLAMA_MODELS_DIR="$new_path"
    echo "OLLAMA_MODELS=$new_path" >> "$CONFIG_FILE"

    # Patch the systemd unit if it exists and contains an Environment= line
    local unit="/etc/systemd/system/ollama.service"
    if [[ -f "$unit" ]]; then
        if grep -q "OLLAMA_MODELS=" "$unit" 2>/dev/null; then
            sudo sed -i "s|OLLAMA_MODELS=.*|OLLAMA_MODELS=$new_path|" "$unit"
        else
            # Insert after the first Environment= line, or add one
            if grep -q "^Environment=" "$unit"; then
                sudo sed -i "/^Environment=/a Environment=\"OLLAMA_MODELS=$new_path\"" "$unit"
            else
                sudo sed -i "/^\[Service\]/a Environment=\"OLLAMA_MODELS=$new_path\"" "$unit"
            fi
        fi
    ...
    log_success "Models moved to $new_path"
    pause
}

# Create a symlink at the OLD default path that points to wherever the models
# actually live.  Handy when third-party tooling hard-codes ~/.ollama/models.
storage_symlink() {
    clear
    echo -e "${CYAN}=== Storage — Create Symlink ===${STD}"
    echo ""
    echo "  This creates a symlink so that a legacy path points to wherever"
    echo "  models actually live now.  Useful when other tools hard-code the"
    echo "  default ~/.ollama/models location."
    echo ""
    echo -e "  Current models dir : ${CYAN}$OLLAMA_MODELS_DIR${STD}"
    echo ""

    read -r -p "  Symlink location (e.g. $HOME/.ollama/models): " link_path
    ...
    if [[ "$link_path" == "$OLLAMA_MODELS_DIR" ]]; then
        log_error "Symlink source and target are the same"
        pause
        return
    fi

    # If something already exists at link_path, handle it
    if [[ -L "$link_path" ]]; then
        local current_target
        current_target=$(readlink -f "$link_path")
        echo -e "  ${YELLOW}A symlink already exists there → $current_target${STD}"
        if ! confirm_action "  Replace it?"; then
            return
        fi
        rm -f "$link_path"
    elif [[ -d "$link_path" ]]; then
        echo -e "  ${RED}A real directory exists at $link_path.${STD}"
        echo "  Move or remove it first, then try again."
        pause
        return
    elif [[ -f "$link_path" ]]; then
        echo -e "  ${RED}A file exists at $link_path.${STD}"
        pause
        return
    fi
    ...
    if ln -s "$OLLAMA_MODELS_DIR" "$link_path"; then
        log_success "Symlink created: $link_path  →  $OLLAMA_MODELS_DIR"
    else
...
# Walk the blobs directory and flag any SHA-256 blob that is not referenced
# by any manifest.  Lets the user delete them to reclaim space.
storage_scan_orphans() {
    clear
    echo -e "${CYAN}=== Storage — Scan for Orphaned Blobs ===${STD}"
    echo ""

    local blobs_dir="$OLLAMA_MODELS_DIR/blobs"
    local manifests_dir="$OLLAMA_MODELS_DIR/models"

    if [[ ! -d "$blobs_dir" ]]; then
        echo -e "  ${YELLOW}No blobs directory found at $blobs_dir${STD}"
        pause
        return
    fi

    echo -e "  Scanning ${CYAN}$blobs_dir${STD} …"

    # 1) Collect every digest that appears in any manifest
    local -A referenced
    while IFS= read -r -d '' manifest; do
        # Manifests reference blobs as "digest":"sha256:abc123…"
        while IFS= read -r digest; do
            referenced["$digest"]=1
        done < <(grep -oP 'sha256:[a-f0-9]+' "$manifest" 2>/dev/null)
    done < <(find "$manifests_dir" -type f -name "*.json" -print0 2>/dev/null)

    # 2) Walk blobs; anything not in the set is orphaned
    local orphan_files=()
    local orphan_total=0
    while IFS= read -r -d '' blob; do
        local bname
        bname=$(basename "$blob")
        if [[ -z "${referenced[$bname]+_}" ]]; then
            orphan_files+=("$blob")
            orphan_total=$(( orphan_total + $(stat -f%z "$blob" 2>/dev/null || stat -c%s "$blob" 2>/dev/null || echo 0) ))
        fi
    done < <(find "$blobs_dir" -type f -print0 2>/dev/null)

    if [[ ${#orphan_files[@]} -eq 0 ]]; then
        echo -e "  ${GREEN}No orphaned blobs found – storage is clean.${STD}"
        pause
        return
    fi
    ...
    # Pretty-print size
    local pretty_total
    if (( orphan_total > 1073741824 )); then
        pretty_total=$(echo "scale=2; $orphan_total / 1073741824" | bc)" GB"
    elif (( orphan_total > 1048576 )); then
        pretty_total=$(echo "scale=2; $orphan_total / 1048576" | bc)" MB"
    else
        pretty_total=$(echo "scale=2; $orphan_total / 1024" | bc)" KB"
    fi

    echo ""
    echo -e "  ${YELLOW}Found ${#orphan_files[@]} orphaned blob(s)  –  reclaimable: ${pretty_total}${STD}"
    echo ""
    for f in "${orphan_files[@]}"; do
        ...
        if (( sz > 1073741824 )); then
            psz=$(echo "scale=2; $sz / 1073741824" | bc)" GB"
        elif (( sz > 1048576 )); then
            psz=$(echo "scale=2; $sz / 1048576" | bc)" MB"
        else
            psz=$(echo "scale=2; $sz / 1024" | bc)" KB"
        fi
        echo -e "    ${RED}$psz${STD}  $(basename "$f")"
    done
    echo ""

    if confirm_action "  Delete all orphaned blobs and reclaim ${pretty_total}?"; then
        log_msg "Deleting ${#orphan_files[@]} orphaned blobs"
        for f in "${orphan_files[@]}"; do
            rm -f "$f"
        done
        log_success "Deleted ${#orphan_files[@]} orphaned blob(s), reclaimed ${pretty_total}"
    fi
    pause
}

# Let the user set a soft warning threshold (GB) and show a status line every
# time the storage menu is entered.  Not enforced – purely advisory.
storage_quota() {
    clear
    echo -e "${CYAN}=== Storage — Quota / Warning Threshold ===${STD}"
    echo ""

    local quota_file="$HOME/.ollama-manager/quota_gb"
    local current_quota="not set"
    [[ -f "$quota_file" ]] && current_quota="$(cat "$quota_file") GB"

    echo -e "  Current warning threshold : ${CYAN}${current_quota}${STD}"

    read -r used total avail pct <<< "$(get_disk_info "$OLLAMA_MODELS_DIR")"
    local model_usage
    model_usage=$(du -sb "$OLLAMA_MODELS_DIR" 2>/dev/null | cut -f1)
    # Convert to GB (float via awk)
    local model_gb
    model_gb=$(awk "BEGIN {printf \"%.2f\", $model_usage / 1073741824}")
    echo -e "  Models currently using    : ${CYAN}${model_gb} GB${STD}"
    echo -e "  Disk free                 : ${CYAN}${avail}${STD}  (${pct} used)"
    echo ""
    echo " 1. Set warning threshold (GB)"
    echo " 2. Clear warning threshold"
    echo " 3. Back"
    echo ""
    read -r -p " → Select option: " choice
    ...
    case $choice in
        1)
            read -r -p "  Enter threshold in GB (e.g. 50): " thresh
            if [[ "$thresh" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
                echo "$thresh" > "$quota_file"
                log_success "Warning threshold set to ${thresh} GB"
            else
        ...
        2)
            rm -f "$quota_file"
            log_success "Warning threshold cleared"
            ;;
        3) return ;;
        *) echo "Invalid option" ;;
    esac
...
# ── main storage sub-menu ────────────────────────────────────────────────────

storage_menu() {
    while true; do
        clear
        echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${STD}"
        echo -e "${CYAN}║          📂  MODEL STORAGE MANAGEMENT                        ║${STD}"
        echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${STD}"
        echo ""

        # Live quota warning check
        local quota_file="$HOME/.ollama-manager/quota_gb"
        if [[ -f "$quota_file" ]]; then
            local quota_gb
            quota_gb=$(cat "$quota_file")
            local model_usage
            model_usage=$(du -sb "$OLLAMA_MODELS_DIR" 2>/dev/null | cut -f1)
            local model_gb
            model_gb=$(awk "BEGIN {printf \"%.2f\", $model_usage / 1073741824}")
            local over
            over=$(awk "BEGIN {print ($model_gb > $quota_gb) ? 1 : 0}")
            if [[ "$over" -eq 1 ]]; then
                echo -e "  ${RED}⚠  QUOTA WARNING: models use ${model_gb} GB  /  threshold ${quota_gb} GB${STD}"
                echo ""
            fi
        fi

        # Compact one-line status
        echo -e "  ${BOLD}Models dir :${STD} ${CYAN}$OLLAMA_MODELS_DIR${STD}"
        if [[ -d "$OLLAMA_MODELS_DIR" ]]; then
            read -r used total avail pct <<< "$(get_disk_info "$OLLAMA_MODELS_DIR")"
            echo -e "  ${BOLD}Disk       :${STD} used ${GREEN}${used}${STD}  total ${GREEN}${total}${STD}  free ${GREEN}${avail}${STD}  (${pct} used)"
            echo -e "  ${BOLD}Models     :${STD} $(du -sh "$OLLAMA_MODELS_DIR" 2>/dev/null | cut -f1)"
        fi
        echo ""

        echo -e "  ${GREEN} 1.${STD} View models & disk usage"
        echo -e "  ${GREEN} 2.${STD} Move models to another directory"
        echo -e "  ${GREEN} 3.${STD} Create symlink (legacy path helper)"
        echo -e "  ${GREEN} 4.${STD} Scan & remove orphaned blobs"
        echo -e "  ${GREEN} 5.${STD} Quota / warning threshold"
        echo -e "  ${GREEN} 6.${STD} Fix permissions (ownership)"
        echo -e "  ${WHITE} 9.${STD} Back to main menu"
        echo ""
        read -r -p " → Select option: " choice
...
Storage Management:
  --storage           Open the interactive storage sub-menu
  --storage-view      Show models directory contents & disk usage
  --storage-move      Move models to a new directory (interactive)
  --storage-orphans   Scan and remove orphaned blobs
...
show_main_menu() {
    ...
    while true; do
        ...
        echo ""
        echo -e " ${YELLOW}╔═══════════════════════════════╗${STD}"
        echo -e " ${YELLOW}║     6. MODEL STORAGE          ║${STD}"
        echo -e " ${YELLOW}╚═══════════════════════════════╝${STD}"
        printf "  %-33s\n" "60. Storage Management"
        echo ""
        echo -e " ${YELLOW}╔═══════════════════════════════╗${STD}"
        echo -e " ${YELLOW}║     7. SYSTEM                 ║${STD}"
        echo -e " ${YELLOW}╚═══════════════════════════════╝${STD}"
        ...
        echo ""
        read -r -p " → Select option or enter command: " choice
        lastmessage="Ready"

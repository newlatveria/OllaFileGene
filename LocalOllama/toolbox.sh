...
maintenance_update_system() {
    ...
    # If it failed and we're on apt, apply mirror-sync recovery
    if [[ $UPDATE_EXIT -ne 0 && "$PKGMGR" == "apt" ]]; then
        echo ""
        echo -e "${YELLOW}Update encountered errors. Attempting recovery...${STD}"
        
        # Retry with --fix-missing (tolerates partially synced mirrors)
        echo "Retrying with --fix-missing..."
        UPDATE_OUTPUT=$(sudo apt update --fix-missing 2>&1)
        UPDATE_EXIT=$?
        echo "$UPDATE_OUTPUT"
        
        # If still failing and the error mentions backports, offer to disable it
        if [[ $UPDATE_EXIT -ne 0 ]] && echo "$UPDATE_OUTPUT" | grep -qi "backports"; then
            echo ""
            echo -e "${YELLOW}The noble-backports mirror appears to be out of sync.${STD}"
            if confirm_action "Temporarily disable backports and retry?"; then
                create_backup "/etc/apt/sources.list" "sources_list_pre_backports_disable"
                sudo sed -i 's/^deb[[:space:]].*noble-backports/#&/' /etc/apt/sources.list
                log_success "Backports disabled in sources.list"
                echo "Retrying update..."
                sudo apt update 2>&1
                UPDATE_EXIT=$?
                echo ""
                echo -e "${YELLOW}Note: Re-enable backports later by editing /etc/apt/sources.list${STD}"
                echo -e "${YELLOW}      or restoring from: $BACKUP_DIR/sources_list_pre_backports_disable_${TIMESTAMP}${STD}"
            fi
        fi
        
        # If we still can't update, bail out
        if [[ $UPDATE_EXIT -ne 0 ]]; then
            log_error "Update failed after recovery attempts"
            pause
            return 1
        fi
    ...
    # Migrate any Intel oneAPI keys stuck in the legacy trusted.gpg keyring
    if [[ -f /etc/apt/trusted.gpg ]] && sudo apt-key list --keyring /etc/apt/trusted.gpg 2>/dev/null | grep -qi "intel"; then
        echo ""
        echo -e "${YELLOW}Migrating legacy Intel GPG key to modern keyring...${STD}"
        INTEL_KEY=$(sudo apt-key list --keyring /etc/apt/trusted.gpg 2>/dev/null | grep -B1 -i "intel" | grep "pub" | awk '{print $2}')
        if [[ -n "$INTEL_KEY" ]]; then
            sudo apt-key export "$INTEL_KEY" 2>/dev/null | sudo gpg --yes --dearmor -o /usr/share/keyrings/oneapi-archive-keyring.gpg
            # Update the oneAPI list file to point at the new keyring if it exists
            if [[ -f /etc/apt/sources.list.d/oneAPI.list ]]; then
                sudo sed -i 's|signed-by=[^]]*|signed-by=/usr/share/keyrings/oneapi-archive-keyring.gpg|' /etc/apt/sources.list.d/oneAPI.list
            fi
            log_success "Intel key migrated — legacy warning should be gone on next update"
        fi
    fi
...
dev_ollama_update() {
    echo -e "${MAGENTA}--- Ollama Update ---${STD}"
    
    if ! command -v ollama &> /dev/null; then
        log_error "Ollama is not installed. Use 'Ollama AI Setup' to install it first."
        pause
        return
    fi
    ...
    # Fetch latest version from the Ollama releases API
    echo "Checking for latest version..."
    local latest_version
    latest_version=$(curl -s https://api.github.com/repos/ollama/ollama/releases/latest \
        | grep -oP '"tag_name":\s*"\K[^"]+' | sed 's/^v//')
    
    if [[ -z "$latest_version" ]]; then
        log_error "Could not fetch latest version from GitHub"
        echo "Falling back to re-running the official installer..."
        if confirm_action "Run the official installer anyway?"; then
            # Stop service before updating
            sudo systemctl stop ollama 2>/dev/null
            curl -fsSL https://ollama.com/install.sh | sh
            sudo systemctl start ollama 2>/dev/null
            log_success "Ollama updated (installer method)"
        fi
        pause
        return
    fi
    ...
    if [[ "$current_version" == "$latest_version" ]]; then
        echo ""
        echo -e "${GREEN}Ollama is already up to date (${current_version})!${STD}"
        pause
        return
    fi
    
    echo ""
    echo -e "${YELLOW}Update available: ${current_version} → ${latest_version}${STD}"
    
    if ! confirm_action "Update Ollama now?"; then
        pause
        return
    fi
    ...
    echo "Downloading and installing Ollama ${latest_version}..."
    curl -fsSL https://ollama.com/install.sh | sh
    
    if [[ $? -ne 0 ]]; then
        log_error "Update failed during installation"
        echo "Attempting to restart the service with the previous version..."
        sudo systemctl start ollama 2>/dev/null
        pause
        return
    fi
    ...
    if [[ "$new_version" == "$latest_version" ]]; then
        log_success "Ollama updated successfully: ${current_version} → ${new_version}"
    else
        echo -e "${YELLOW}Version check after update: ${new_version}${STD}"
        echo "(Note: Version string may differ slightly from GitHub tag)"
        log_success "Ollama update completed"
    fi
    
    # Verify service is healthy
    echo ""
    echo -e "${CYAN}Verifying service health...${STD}"
    if curl -s http://localhost:11434/api/version >/dev/null 2>&1; then
        log_success "Ollama service is running and responding"
    else
        log_error "Ollama service is not responding after update"
        echo "Try restarting manually: sudo systemctl restart ollama"
    fi
...
handle_cli_args() {
    case "$1" in
        --help|-h)
            ...
            echo "  --install-tools       Install core system tools"
            echo "  --update              Update system packages"
            echo "  --cleanup             Clean system (logs, cache, temp)"
            echo "  --kill-zombies        Kill zombie processes"
            echo ""
            echo "─── Rescue & Recovery ───────────────────────────────────"
            echo "  --auto-diag           Run auto-diagnostic & repair"
            echo "  --grub-cheatsheet     Print GRUB rescue command reference"
            echo "  --disk-analyze        Launch interactive disk analyzer"
            echo ""
            echo "─── Dev, AI & Containers ────────────────────────────────"
            echo "  --ollama-setup        Configure Ollama & GPU integration"
            echo "  --ollama-update       Update Ollama to latest version"
            echo "  --install-go          Install latest Go toolchain"
            echo ""
            echo "─── Hardware & Android ──────────────────────────────────"
            echo "  --gpu-info            Display GPU information"
            echo "  --gpu-monitor         Launch live GPU monitor"
            echo "  --stress-test         Launch stress test suite"
            echo "  --android             Launch Android device manager"
            echo "  --network             Launch network tools"
            echo "  --system-info         Display full system information"
            echo ""
            echo "─── Backup & Restore ────────────────────────────────────"
            echo "  --backup-home         Backup home directory"
            echo "  --backup-etc          Backup system configuration (/etc)"
            echo "  --backup-packages     Save installed package list"
            echo ""
            echo "─── General ─────────────────────────────────────────────"

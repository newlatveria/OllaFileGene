...
with t_chat:
    ...
    # Right: The IDE (Full CRUD)
    with col_exec:
        st.subheader("🛠️ IDE Workbench")
        
        files = [f for f in os.listdir(WORKSPACE) if os.path.isfile(os.path.join(WORKSPACE, f))]
        c_nav, c_load = st.columns([2, 1])
        with c_nav:
            selected_file = st.selectbox("📁 Workspace Files", ["New File"] + files)
        with c_load:
            if selected_file != "New File" and st.button("📖 Load", use_container_width=True):
                with open(os.path.join(WORKSPACE, selected_file), "r") as f:
                    st.session_state["latest_code"] = f.read()
                    st.rerun()

        # Editor
        code_editor = st.text_area("Source Code", value=st.session_state["latest_code"], height=400, key="main_editor")
        st.session_state["latest_code"] = code_editor
        save_name = st.text_input("File Name:", value=selected_file if selected_file != "New File" else "script.py")
        
        # CRUD & Exec Actions
        b1, b2, b3, b4 = st.columns(4)
        if b1.button("💾 Save", use_container_width=True):
            with open(os.path.join(WORKSPACE, os.path.basename(save_name)), "w") as f:
                f.write(code_editor)
            st.toast("File Saved!")
            st.rerun()
        if b2.button("🔍 Audit", use_container_width=True):
            st.code(run_shell(["venv/bin/bandit", "-r", os.path.join(WORKSPACE, save_name)]))
        if b3.button("▶️ Run", use_container_width=True):
            if any(t in code_editor for t in DANGEROUS_TOKENS): st.error("Blocked dangerous code.")

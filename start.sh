...
start_streamlit() {
    ...
    if [[ ! -f "$SCRIPT_DIR/llm_model_factory_secure.py" ]]; then
        print_error "Application file not found: llm_model_factory_secure.py"
...
start_with_logs() {
    ...
    nohup streamlit run llm_model_factory_secure.py \
        --server.address 0.0.0.0 \

...
print_header() {
    clear
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}${MAGENTA}     🔐 Secure LLM Model Factory - Setup & Start${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════════${NC}"
    echo ""
}
...
check_and_install_dependencies() {
    ...
    if [[ -f "$SCRIPT_DIR/requirements.txt" ]]; then
        ...
        pip install streamlit requests psutil PyPDF2 python-docx -q
    ...
    if python3 -c "import psutil" 2>/dev/null; then
        print_success "psutil verified"
    else
        print_error "psutil installation failed"
        exit 1
    fi

        if python3 -c "import python-docx" 2>/dev/null; then
        print_success "python-docx verified"
    else
        print_error "python-docx installation failed"
        pause
    fi

    if python3 -c "import PyPDF2" 2>/dev/null; then
        print_success "PyPDF2 verified"
    else
        print_error "PyPDF2 installation failed"
        pause
    fi
...
start_streamlit() {
    ...
    if [[ ! -f "$SCRIPT_DIR/llm_model_factory_complete_fixed.py" ]]; then
        print_error "Application file not found: llm_model_factory_complete_fixed.py"
...
start_with_logs() {
    ...
    nohup streamlit run llm_model_factory_complete_fixed.py \
        --server.address 0.0.0.0 \
